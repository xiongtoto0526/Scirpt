...
// @ngInject
// Refine Tips, No show when no value
    ...
    function chart(selection) {
      ...
      selection.each(function(data) {
        ...
        interactiveLayer.dispatch.on('elementMousemove', function(e) {
          ...
          // Start Refine Tips, No show when no value
          var singlePoint;
          var pointIndex;
          var pointXLocation;
          var allData = [];
          var maxLength = _.max(_.pluck(data, 'values'), function(item) {
              return item.length;
            }).length;
          angular.copy(data)
            .filter(function(series, i) {
              series.seriesIndex = i;
              if (series.values && series.values[0]) {
                for (var i = series.values.length; i < maxLength; i++) {
                  series.values[i] = {};
                  _.each(series.values[0], function(v, k) {
                    if (k === chart.xName()) {
                      for (var j = 0; j < data.length; j++) {
                        if (data[j].values[i]) {
                          series.values[i][k] = data[j].values[i][k];
                          break;
                        }
                      }
                    } else if (k === chart.yName()) {
                      series.values[i][k] = null;
                    } else {
                      series.values[i][k] = series.values[0][k];
                    }
                  });
                }
              }
            ...
            // End Refine Tips, No show when no value
            .forEach(function(series, i) {
              ...
              // Start Refine Tips, No show when no value
              if (pointYValue !== null && pointYValue != undefined) {
                allData.push({
                  key: series.key,
                  value: pointYValue,
